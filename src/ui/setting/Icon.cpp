...
QPixmap Icon::GetTrayIcon(TrayIconStatus status) {
    ...
    if (status == NONE)
    {
        ...
        if (pixmap_read.isNull()) {
            pixmap_read = QPixmap(QString(":/FreeLink/") + "Off" + ".png");
        }
        if (!pixmap_read.isNull()) pixmap = pixmap_read;
    } else if (status == RUNNING)
    {
        if (Configs::dataManager->settingsRepo->use_custom_icons) {
            pixmap_read = QPixmap(QString("icons/") + "FreeLink" + ".png");
        }
        if (pixmap_read.isNull()) {
            pixmap_read = QPixmap(QString(":/FreeLink/") + "FreeLink" + ".png");
        }
        if (!pixmap_read.isNull()) pixmap = pixmap_read;
    } else if (status == SYSTEM_PROXY_DNS)
    {
        ...
        if (pixmap_read.isNull()) {
            pixmap_read = QPixmap(QString(":/FreeLink/") + "Proxy-Dns" + ".png");
        }
        if (!pixmap_read.isNull()) pixmap = pixmap_read;
    } else if (status == SYSTEM_PROXY)
    {
        ...
        if (pixmap_read.isNull()) {
            pixmap_read = QPixmap(QString(":/FreeLink/") + "Proxy" + ".png");
        }
        if (!pixmap_read.isNull()) pixmap = pixmap_read;
    } else if (status == DNS)
    {
        ...
        if (pixmap_read.isNull()) {
            pixmap_read = QPixmap(QString(":/FreeLink/") + "Dns" + ".png");
        }
        if (!pixmap_read.isNull()) pixmap = pixmap_read;
    } else if (status == VPN)
    {
        ...
        if (pixmap_read.isNull()) {
            pixmap_read = QPixmap(QString(":/FreeLink/") + "Tun" + ".png");
        }
        if (!pixmap_read.isNull()) pixmap = pixmap_read;
    } else
    {
