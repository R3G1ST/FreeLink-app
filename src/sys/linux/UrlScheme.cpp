...
static const QString kDesktopId = "freelink-url-handler.desktop";
...
void UrlScheme_Apply() {
    ...
    if (f.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        ts << "[Desktop Entry]\n"
           ...
           << "Name=FreeLink\n"
           << "Exec=\"" << execTarget() << "\" %u\n"
           << "MimeType=x-scheme-handler/freelink;\n"
           << "Terminal=false\n"
           << "NoDisplay=true\n";
        ts.flush();
        f.close();
    ...
    QProcess::execute("xdg-mime", {"default", kDesktopId, "x-scheme-handler/freelink"});
}
